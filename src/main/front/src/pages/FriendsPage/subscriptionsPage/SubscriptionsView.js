...
useEffect(() => {
  dispatch(getFriendSubscriptions())
}, [dispatch]);
    ...
    useEffect(() => {
        if (friendsSl.subscriptionsList) {
...
function addFriendHandler(user) {
  ...
  dispatch(addFriendSubscribe(myData))
    ...
    const result = friendsSl.subscriptionsList?.map((el, i) => {
        return (
    ...
    return (
        <div className={style.tablemain}>
            ...
            <div className={style.display_info}>
                <p>Всего найдено: {friendsSl.subscriptionsList?.length}</p>
            </div>
